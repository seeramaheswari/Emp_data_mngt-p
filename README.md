...
```
select extract(year from hire_date) as join_year ,extract(month from hire_date) 
as join_month ,count(*) from employees group by 1,2 order by 1,2 desc;
```
...
```
select e.employee_id,s.bonus from employees as e inner join salaries 
as s on e.employee_id=s.employee_id where s.bonus>200 and s.bonus<1000 order by s.bonus desc ;
```
***-- Q.6 Write sql query to check count employees in each month that got payment***

>select extract(month from payment_date) as month,count(*) from payroll group by 1;

***-- Q.7 Write a sql query to which position get highest salary***
```
WITH RankedSalaries as ( select e.position,s.base_salary, dense_rank() over (order by s.base_salary desc) 
as salary_rank from employees as e inner join salaries as s on e.employee_id=s.employee_id ) 
select position,base_salary from RankedSalaries where salary_rank=1;
```
